...
prompts = [
    ...
    "This is a self-driving scene at night."
    ]
...
def classify_day_night_clip(image_path):
    ...
    # 返回最高得分的类别
    if probs[0][0] > probs[0][1] and probs[0][0] > 0.8:
        label = "day"
...
images_json = "s3://sdagent-shard-bj-baiducloud/wheeljack/wuxiaolei/img_translation/data/vlm_all_img_data.json"
img_path_list = json.load(refile.smart_open(images_json))
...
for idx in tqdm(range(begin, len(front_path_list)-1, step), desc="[processing...]"):
    image_path = front_path_list[idx]
